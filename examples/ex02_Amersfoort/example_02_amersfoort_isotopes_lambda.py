...
# -*- coding: utf-8 -*-
"""
Lambda plot of isotopes reproducing figure 8 in Paper van Leeuwen et al., 2022
...
isotopes_raw,units = md.load_excel(file_path,
                                   sheet_name = 'isotopes',
                                   verbose = verbose)

isotopes,units = md.standardize(isotopes_raw,
                                reduce = True,
                                verbose=verbose)
...
for j,molecule in enumerate(molecules): 
    ...
    if j%2 == 0:        
        ax[j].set_ylabel('$\delta^2$H')

    if j >= len(ax)-2:        
        ax[j].set_xlabel('$\delta^{{13}}$C')
