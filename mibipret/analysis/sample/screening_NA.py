...
def oxidators(
    ...
    **kwargs,
    ...
    based on nitrogen and phosphate availability
    making use of molecular mass values for quantities in [mg/mmol]
    ...
    for cont in ean.contaminants[contaminant_group]:

        if cont in cols:
            # tot_oxi += data[cont]* 0.001/properties[cont]['molecular_mass'] *  properties[cont]['factor_stoichiometry'] 
...
def NA_traffic(
    ...
    if verbose:
        ...
        print("Red light: Reduction is limited at {} out of {} locations".format(np.sum(traffic == "red"),len(e_bal)))
        print("Green light: Reduction is limited at {} out of {} locations".format(np.sum(traffic == "green"),len(e_bal)))
        print("Yellow light: No decision possible at {} out of {} locations".format(np.sum(np.isnan(e_bal)),len(e_bal)))
...
def thresholds_for_intervention(
    ...
    if verbose:
        print("===============================================================")
        print("Evaluation of contaminant concentrations exceeding intervention values for {}:\n".format(contaminant_group))
        print("Red light: Intervention values exceeded for {} out of {} locations".format(np.sum(traffic >0),data.shape[0]))
        print("green light: Concentrations below intervention values at {} out of {} locations".format(np.sum(traffic == 0),data.shape[0]))
        print("Yellow light: No decision possible at {} out of {} locations".format(np.sum(np.isnan(traffic)),data.shape[0]))
...
def screening_NA(
    ...
    **kwargs,
    ...
    for add in [na_traffic,e_bal,tot_oxi,tot_reduct,tot_cont]:
